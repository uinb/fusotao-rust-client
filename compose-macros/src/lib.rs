    ...
    ($node_metadata: expr, $pallet: expr, $call_name: expr $(, $args: expr) *) => {
        {
            ...
            ([pallet.index, *call_index as u8] $(, ($args)) *).encode()
        }
    };
}
...
/// * 'call' - call as returned by the compose_raw_call! macro or via substrate's call enums.
...
#[macro_export]
macro_rules! compose_raw_extrinsic_offline {
    ($signer: expr,
    $call: expr,
    $nonce: expr,
    $era: expr,
    ...
    $transaction_version: expr) => {{
        use $crate::primitives::{GenericAddress, GenericExtra, RawExtrinsic, RawSignedPayload};
        use $crate::sp_runtime::{generic::Era, traits::IdentifyAccount, MultiSigner};

        let extra = GenericExtra::new($era, $nonce);
        let mut raw_payload = RawSignedPayload::from_raw(
            $call.clone(),
            extra.clone(),
            (
        ...
        let signature = raw_payload.sign(|payload| $signer.sign(payload));

        let multi_signer: MultiSigner = $signer.public().into();

        RawExtrinsic::new_signed(
            $call,
	...
	$(, $args: expr) *) => {
		{
            ...
            let call = $crate::compose_raw_call!($api.metadata, $module, $call $(, ($args)) *);

            if let Some(signer) = $api.signer.clone() {
                $crate::compose_raw_extrinsic_offline!(
                    signer,
                    call,
